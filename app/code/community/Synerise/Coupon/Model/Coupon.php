...
class Synerise_Coupon_Model_Coupon extends Mage_Core_Model_Abstract
{
    ...
    protected function _getMageCoupon($force = false)
    {
        if($force || !$this->_mageCoupon instanceof Mage_SalesRule_Model_Coupon) {
            $this->_mageCoupon = Mage::getModel('salesrule/coupon')->load($this->getCouponCode(), 'code');
        }
...
//            ->setDiscountStep(0);
    ...
    public function isSyneriseCoupon() 
    {
        ...
        if(!$this->validateFormat($this->getCouponCode())) {
            return false;
        }
